...
| D20 | Auth library | Better Auth 1.7 with the Prisma adapter, Argon2id via its custom `password.hash/verify`, plugins `bearer` (mobile), `two-factor` (TOTP), `haveibeenpwned`; Redis `secondaryStorage` for rate limits. Better Auth stores session tokens in plain text, so a thin adapter wrapper hashes `session.token` (SHA-256) on write and on lookup to meet SECURITY.md; covered by tests in 1A.2 | Passport + hand-written sessions | Covers email, OAuth, 2FA, bearer and rate limiting out of the box; the one SECURITY.md gap is closable in a small, testable wrapper. Decided in 1A.1 because Account/Session shape depends on it |
