...
| 0.4 | Server audit and PaaS setup: inventory what runs on dok.seil.products, confirm Dokploy vs Coolify, create `photoo-staging` and `photoo-prod` projects, DNS for photoo.lu / api. / admin. / staging.* , TLS, firewall + SSH hardening per `SECURITY.md` | devops-engineer (+ human for DNS/credentials) | M | – |
