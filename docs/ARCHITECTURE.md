...
- The scoped database is cloned from the already-migrated-and-seeded `TEST_DATABASE_URL` database via `CREATE DATABASE ... TEMPLATE` (an OS-level file copy, not a re-run of migrate+seed) and re-cloned automatically if the template's migration history has moved on since the clone was made, guarded by a Postgres advisory lock on the template name so `packages/db`/`apps/api`/`apps/worker`'s concurrent clone attempts never race a `CREATE DATABASE ... TEMPLATE` against another process's connection to the source.
- The scoped Redis URL picks one of Redis's 16 logical databases (`SELECT`) by hashing `(worktree root, workspace)`; unlike the Postgres clone this is not structurally collision-free (16 slots is a small keyspace once several worktrees are active at once), but the same pair always hashes to the same index, so a collision is deterministic and debuggable rather than flaky. It isolates BullMQ queue names (`apps/api`'s `TestEmailWorkerModule` and `apps/worker`'s `QueueWorkersService` both run a real `Worker` on `EMAIL_QUEUE_NAME`) and rate-limit/lockout keys, but not Socket.IO's redis-adapter pub/sub, which Redis broadcasts across every logical database regardless of `SELECT` — see `apps/api/src/modules/chat/socket-io-redis-adapter.ts`'s own comment for that residual gap.
