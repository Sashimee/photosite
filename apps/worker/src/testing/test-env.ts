import type { Env } from '../config/env.js';

// vitest.config.ts scopes and reassigns TEST_DATABASE_URL/REDIS_URL before this module loads; the fallbacks below only apply when neither var is set.
export const TEST_ENV: Env = {
  NODE_ENV: 'test',
  DATABASE_URL:
    process.env.TEST_DATABASE_URL ?? 'postgresql://user:pass@127.0.0.1:5432/photoo_test',
  REDIS_URL: process.env.REDIS_URL ?? 'redis://127.0.0.1:6379',
  WEB_APP_URL: 'http://localhost:3000',
  HEALTH_PORT: 4101,
  CLAMAV_HOST: '127.0.0.1',
  CLAMAV_PORT: 3310,
  CLAMAV_SCAN_TIMEOUT_MS: 30_000,
  CLAMAV_MAX_SCAN_BYTES: 25 * 1024 * 1024,
  WORKER_CONCURRENCY_FILE_SCAN: 2,
  WORKER_CONCURRENCY_IMAGE_PROCESS: 2,
  WORKER_CONCURRENCY_UPLOADS_CLEANUP: 1,
  UPLOADS_CLEANUP_INTERVAL_MS: 10 * 60 * 1000,
  WORKER_CONCURRENCY_QUOTE_EXPIRY: 1,
  QUOTE_EXPIRY_INTERVAL_MS: 15 * 60 * 1000,
  WORKER_CONCURRENCY_LISTING_EXPIRY: 1,
  LISTING_EXPIRY_INTERVAL_MS: 60 * 60 * 1000,
  IMAGE_PROCESS_MAX_PIXELS: 100_000_000,
  WORKER_CONCURRENCY_PROVENANCE_CHECK: 2,
  PROVENANCE_ENABLED: false,
  PROVENANCE_C2PA_ENABLED: false,
  PROVENANCE_AI_DETECTION_API_KEY: undefined,
  PROVENANCE_REVERSE_SEARCH_API_KEY: undefined,
  S3_ENDPOINT: 'http://127.0.0.1:9000',
  S3_REGION: 'eu-west-1',
  S3_ACCESS_KEY_ID: 'photoo_dev',
  S3_SECRET_ACCESS_KEY: 'photoo_dev_password',
  S3_FORCE_PATH_STYLE: true,
  S3_PRIVATE_BUCKET: 'photoo-private',
  S3_PUBLIC_BUCKET: 'photoo-public',
  SMTP_HOST: 'localhost',
  SMTP_PORT: 1025,
  SMTP_SECURE: false,
  SMTP_USER: undefined,
  SMTP_PASSWORD: undefined,
  SMTP_FROM: 'dev@photoo.test',
  SMTP_INSECURE_INTERNAL_RELAY: false,
  EXPO_ACCESS_TOKEN: undefined,
  WORKER_CONCURRENCY_EMAIL: 3,
  WORKER_CONCURRENCY_NOTIFY: 3,
  WORKER_CONCURRENCY_NOTIFY_SWEEP: 1,
  WORKER_CONCURRENCY_PUSH_RECEIPTS: 1,
  WORKER_CONCURRENCY_NOTIFICATIONS_CLEANUP: 1,
  NOTIFY_SWEEP_INTERVAL_MS: 5 * 60 * 1000,
  PUSH_RECEIPTS_INTERVAL_MS: 15 * 60 * 1000,
  NOTIFICATIONS_CLEANUP_INTERVAL_MS: 24 * 60 * 60 * 1000,
  WORKER_CONCURRENCY_GDPR_EXPORT: 1,
  WORKER_CONCURRENCY_GDPR_SWEEP: 1,
  GDPR_SWEEP_INTERVAL_MS: 60 * 60 * 1000,
  SENTRY_DSN: undefined,
  SENTRY_ENVIRONMENT: undefined,
  SENTRY_TRACES_SAMPLE_RATE: 0,
  SENTRY_REQUIRED: false,
};

export const UNREACHABLE_TEST_ENV: Env = {
  ...TEST_ENV,
  DATABASE_URL: 'postgresql://user:pass@127.0.0.1:1/photoo_test',
  REDIS_URL: 'redis://127.0.0.1:1',
};
