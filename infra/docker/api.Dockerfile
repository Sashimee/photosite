...
COPY . .
RUN turbo prune @photoo/api --docker
...
RUN pnpm exec turbo run build --filter=@photoo/api...
...
COPY --from=trimmed --chown=app:app /app/apps/api/package.json ./apps/api/package.json
COPY --from=trimmed --chown=app:app /app/apps/api/node_modules ./apps/api/node_modules
COPY --from=trimmed --chown=app:app /app/apps/api/dist ./apps/api/dist
